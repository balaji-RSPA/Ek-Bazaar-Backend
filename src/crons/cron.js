...
exports.sendDailyCount = async (req, res) => new Promise(async (resolve, reject) => {
  try {
    ...
    let source = ["GCC", "SMEC", "Paper Ads", "Online Ads", "Social Media", "From a Friend", "Desh Aur Vyapar", "Tamil Nadu", "Uttar Pradesh", "Uttarakhand"]
    ...
    const sum = gcc_count.length + smec_ount.length + paper_ads_count.length + online_ads_count.length + social_media_count.length + from_a_friend_count.length + desh_or_vyapar_count.length + tamil_nadu_count.length + uttar_pradesh_count.length + uttarakhand_count.length


    source = source.map((src, i) => {
      return ({
        key: src,
        value: i == 0 ? gcc_count.length : i == 1 ? smec_ount.length : i == 2 ? paper_ads_count.length : i == 3 ? online_ads_count.length : i == 4 ? social_media_count.length : i == 5 ? from_a_friend_count.length : i == 6 ? desh_or_vyapar_count.length : i == 7 ? tamil_nadu_count.length : i == 8 ? uttar_pradesh_count.length : uttarakhand_count.length
      })
    })
