...
module.exports.sendOtp = async (req, res) => {
  ...
  module.exports.updateUser = async (req, res) => {
    try {
      const { userID } = req;
      console.log("🚀 ~ file: userController.js ~ line 399 ~ module.exports.updateUser= ~ userID", userID)
    console.log("🚀 ~ file: userController.js ~ line 401 ~ module.exports.updateUser= ~ req.body.", req.body)
      ...
      updateSeller({ userId: userID }, sellerData);
      await updateBuyer({ userId: userID }, buyerData);
      
      let seller = await getSeller(userID)
      if (business) {
        const bsnsDtls = await addbusinessDetails(seller._id, { name: business });
        updateSeller({ userId: userID }, {
          busenessId: bsnsDtls._id,
        });
      }
      let buyer = await getBuyer(userID)
      console.log("🚀 ~ file: userController.js ~ line 441 ~ module.exports.updateUser= ~ user", user)
    console.log("🚀 ~ file: userController.js ~ line 457 ~ module.exports.updateUser= ~ seller", seller)
    console.log("🚀 ~ file: userController.js ~ line 459 ~ module.exports.updateUser= ~ buyer", buyer)
      ...
      if (user && buyer && seller) {
        ...
        if (buyer.isEmailSent === false && buyer.email) {
          ...
          updateBuyer({ _id: buyer._id }, buyer);
          updateSeller({ _id: seller._id }, seller);
        ...
        console.log("aggggggggggg ghantaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        respSuccess(res, {
          seller,
          buyer,
          activeChat
