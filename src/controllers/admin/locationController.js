...
module.exports.getCities = async (req, res) => {
  try {
    const { skip, limit, search, indiaCity, countryId } = req.query
    const cities = await getAllCities({skip:parseInt(skip),limit:parseInt(limit), search, indiaCity,countryId});
