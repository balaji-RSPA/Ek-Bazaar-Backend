...
exports.structureSellerData = async (seller) => {
  ...
  if (sellerExist) {
    ...
    console.log(
      "🚀 ~ file: sellersModule.js ~ line 686 ~ exports.structureSellerData= ~ levelFive.length",
      levelFive
    );
    ...
    // } else {
    console.log(
      "🚀 ~ file: sellersModule.js ~ line 686 ~ exports.structureSellerData= ~ levelFour.length",
      levelFour.length
    );
    ...
    // }
    console.log(
      "🚀 ~ file: sellersModule.js ~ line 708 ~ exports.structureSellerData= ~ productData",
      productData
    );
    ...
    if (result) {
      ...
      console.log(
        "🚀 ~ file: sellersModule.js ~ line 854 ~ exports.structureSellerData= ~ levelFive.length",
        levelFive.length
      );
      // productData = levelFive.map(pro => ({
      //   sellerId: result._id,
      ...
      // } else {
      console.log(
        "🚀 ~ file: sellersModule.js ~ line 865 ~ exports.structureSellerData= ~ levelFour.length",
        levelFour.length
      );
      productData = levelFour.map((pro) => ({
        sellerId: result._id,
      ...
      // }
      console.log(
        "🚀 ~ file: sellersModule.js ~ line 875 ~ exports.structureSellerData= ~ productData",
        productData
      );
      proData = await sellerProductsBulkInsert(productData);
      const upData = {
...
module.exports.getUpdatedSellerDetails = (query, skip, limit) =>
  new Promise((resolve, reject) => {
    Sellers.find(query)
      .skip(skip)
      .limit(limit)
...
exports.sellersOverAllCount = (search) =>
  new Promise((resolve, reject) => {
    Sellers.aggregate([
      {
        $match: {
...
exports.fetchPartiallyRegistredSeller = () =>
  new Promise((resolve, reject) => {
    let now = new Date();
    Sellers.aggregate([
      {
        $match: {
          $and: [
            ...
            { sellerType: { $exists: true, $not: { $size: 0 } } },
            {
              $or: [
                { statutoryId: { $eq: null } },
                { sellerContactId: { $eq: null } },
              ],
            },
          ],
        },
      ...
      .then((doc) => {
        ...
        const data =
          (doc &&
            doc.length &&
            doc.filter(
              (record) => now.diff(record.updatedAt, "minutes") >= 3
            )) ||
          [];
...
exports.fetchPartiallyRegistredBuyer = () =>
  new Promise((resolve, reject) => {
    let now = new Date();
    Buyer.aggregate([
      {
...
const SendSMSNotifc = async (el) => {
  try {
    ...
    let smscountryCode =
      el.countryCode && el.countryCode.length ? el.countryCode : "+91";
    let mobile = el.mobile && el.mobile.length && el.mobile;
    await Buyer.updateOne(
      { _id: el._id },
      { $set: { incompleteRegistration: 2 } }
    );
    const { messagePartial, templateId } = partialRegistred({
      name: el.name || "User",
    });
    ...
    el.mobile &&
      el.mobile.length &&
      smscountryCode &&
      (await sendSMS(`${smscountryCode}${mobile}`, messagePartial, templateId));
    return true;
  } catch (error) {
    console.log(error, "=============");
  }
...
const SendNotifc = async (el) => {
  try {
    ...
    let countryCodeVal =
      ...
      el.mobile.length &&
      el.mobile[0] &&
      el.mobile[0].countryCode &&
      el.mobile[0].countryCode
        ? el.mobile[0].countryCode.replace("+", "")
        : "91";
    let smscountryCode =
      el.mobile &&
      el.mobile.length &&
    ...
    el.mobile &&
      ...
      smscountryCode &&
      (await sendSMS(
        `${smscountryCode}${el.mobile[0].mobile}`,
        messagePartial,
        templateId
      ));
...
module.exports.getSellersListDataAgg = (aggregate) =>
  new Promise((resolve, reject) => {
    Sellers.aggregate(aggregate)
      // .lean()
      .then((doc) => {
        resolve(doc);
      })
...
module.exports.getStates = (aggregate) =>
  new Promise((resolve, reject) => {
    Sellers.find()
      // .lean()
      .then((doc) => {
